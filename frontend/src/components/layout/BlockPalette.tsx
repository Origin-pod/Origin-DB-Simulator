...
function BlockItem({ block, onDragStart }: BlockItemProps) {
  ...
  return (
    ...
    >
      ...
      {/* Complexity badges (always visible when available) */}
      {complexity && (
        <div className="flex gap-1.5 mb-1">
          <span className="text-[10px] bg-white/70 text-gray-600 px-1.5 py-0.5 rounded border border-gray-200 font-mono">
            T: {complexity.time}
          </span>
          <span className="text-[10px] bg-white/70 text-gray-600 px-1.5 py-0.5 rounded border border-gray-200 font-mono">
            S: {complexity.space}
